...
class AbpBlocker(
    ...
    fun shouldBlock(request: ContentRequest): BlockerResponse? {
        ...
        filterContainers[ABP_PREFIX_DENY]!![request]?.let {
            return blockOrRedirect(request,
                ABP_PREFIX_DENY,
                it.pattern)
        }
    ...
    private fun allowOrModify(request: ContentRequest): ModifyResponse? {
        ...
        return getModifiedResponse(request,
            modifyFilters.mapNotNull { it.modify }.toMutableList(),
            remainingExceptions)
    }
    ...
    private fun MutableList<ContentFilter>.removeModifyExceptions(
        request: ContentRequest,
        prefix: String,
    ): List<ModifyFilter> {
        ...
        for (i in (0 until modifyExceptions.size).reversed()) {
            if (modifyExceptions[i].modify!!.parameter == null) {
                ...
                if (modifyExceptions[i].modify!! is ResponseHeaderFilter && modifyExceptions[i].modify!!.parameter?.contains(
                        ':') == false
                ) {
                    val header = modifyExceptions[i].modify!!.parameter!!
                    removeAll {
                        it.modify is ResponseHeaderFilter && it.modify!!.parameter?.startsWith(
                            header) == true && it.modify!!.inverse == modifyExceptions[i].modify!!.inverse
                    }
                    modifyExceptions.removeAt(i) // not needed any more
                }
                // same thing for requestheaders, not sure though whether it is used at all
                else if (modifyExceptions[i].modify!! is RequestHeaderFilter && modifyExceptions[i].modify!!.parameter?.contains(
                        ':') == false
                ) {
                    val header = modifyExceptions[i].modify!!.parameter!!
                    removeAll {
                        it.modify is RequestHeaderFilter && it.modify!!.parameter?.startsWith(header) == true && it.modify!!.inverse == modifyExceptions[i].modify!!.inverse
                    }
                    modifyExceptions.removeAt(i) // not needed any more
                }
            }
    ...
    companion object {
        ...
        private fun getModifiedResponse(
            request: ContentRequest,
            filters: MutableList<ModifyFilter>,
            remainingExceptions: List<ModifyFilter>,
        ): ModifyResponse? {
            ...
            val newUrl = if (parameters == null)
                ...
                request.url.toString().substringBefore('?')
                    .substringBefore('#') + // url without parameters and fragment
        ...
        private fun getModifiedParameters(
            url: Uri,
            filters: List<ModifyFilter>,
            exceptions: List<ModifyFilter>,
        ): Map<String, String>? {
        ...
        class MapEntry(override val key: String, override val value: String) :
        ...
        private fun Uri.getQueryParameterMap(): LinkedHashMap<String, String> {
