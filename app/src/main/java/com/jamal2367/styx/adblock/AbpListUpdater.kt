...
import com.jamal2367.styx.adblock.AbpBlockerManager.Companion.blockerPrefixes
import com.jamal2367.styx.adblock.AbpBlockerManager.Companion.isModify
...
class AbpListUpdater @Inject constructor(val context: Context) {
    ...
    private fun updateHttp(entity: AbpEntity, forceUpdate: Boolean): Boolean {
        ...
        val request = try {
            Request.Builder()
                .url(entity.url)
                .header("User-Agent",
                    userPreferences.userAgent(context.applicationContext as Application))
                .get()
        ...
        if (!forceUpdate) {
            entity.lastModified?.let {
                ...
                if ((blockerPrefixes + ABP_PREFIX_DISABLE_ELEMENT_PAGE + ABP_PREFIX_ELEMENT).map { prefix ->
                        dir.getFilterFile(prefix, entity).exists()
                    }.any()
                )
        ...
        try {
            ...
            if (!response.isSuccessful) {
                Handler(Looper.getMainLooper()).post {
                    context.toast(context.getString(R.string.blocklist_update_error_code,
                        entity.title,
    ...
    private fun decode(reader: BufferedReader, charset: Charset, entity: AbpEntity): Boolean {
        ...
        writer.write(dir.getFilterFile(ABP_PREFIX_DISABLE_ELEMENT_PAGE, entity),
...
//                ErrorReport.printAndWriteLog(e)
    ...
    fun needsUpdate(entity: AbpEntity): Boolean {
        val now = System.currentTimeMillis()
        if (now - entity.lastLocalUpdate >= max(entity.expires * AN_HOUR,
                A_DAY * userPreferences.blockListAutoUpdateFrequency)
        ) {
            return true
        }
        return false
