...
import com.jamal2367.styx.constant.Hosts
import com.jamal2367.styx.constant.Schemes
import com.jamal2367.styx.constant.Uris
import com.jamal2367.styx.constant.WINDOWS_DESKTOP_USER_AGENT_PREFIX
...
import com.jamal2367.styx.preference.webViewEngineVersionDesktop
...
class StyxView(
     ...
     * @return a NonNull instance of StyxViewTitle
     */
    val titleInfo: StyxViewTitle = StyxViewTitle(activity)
        ...
        set(aDesktopMode) {
            ...
            if (aDesktopMode) {
                webView?.settings?.userAgentString =
                    WINDOWS_DESKTOP_USER_AGENT_PREFIX + webViewEngineVersionDesktop(activity.application)
            } else {
    ...
    private val dialogBuilder: StyxDialogBuilder = hiltEntryPoint.dialogBuilder
    val databaseScheduler: Scheduler = hiltEntryPoint.databaseScheduler()
    val mainScheduler: Scheduler = hiltEntryPoint.mainScheduler()
    private val networkConnectivityModel: NetworkConnectivityModel =
        hiltEntryPoint.networkConnectivityModel
    ...
    init {
        ...
        if (!networkConnectivityModel.isNetworkAvailable()) {
            (activity).snackbar(R.string.no_connection)
        }
    }
    ...
    private fun createWebView() {
        ...
        webView = LayoutInflater.from(activity).inflate(R.layout.webview, container, false) as WebView
        webView?.apply {
            setFindListener(this@StyxView)
            //id = this@StyxView.id
        ...
        if (currentUrl != null && currentUrl.isSpecialUrl()) {
            if (currentUrl.isHistoryUrl()) {
                ...
                if (url != null) {
                    ...
                    dialogBuilder.showLongPressedDialogForBookmarkUrl(activity,
                        uiController,
                        newUrl)
                }
