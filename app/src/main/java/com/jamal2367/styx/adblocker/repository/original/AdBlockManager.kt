...
package com.jamal2367.styx.adblocker.repository.original
...
import com.jamal2367.styx.adblocker.filter.fastmatch.AdBlockDecoder
import com.jamal2367.styx.adblocker.filter.fastmatch.LegacyDecoder
...
import com.jamal2367.styx.adblocker.filter.unified.io.FilterReader
import com.jamal2367.styx.adblocker.filter.unified.io.FilterWriter
import com.jamal2367.styx.adblocker.filter.unified.writeFilter
...
class AdBlockManager internal constructor(private val context: Context) {
    ...
    private fun updateInternal(table: String, adBlock: AdBlock): Boolean {
        ...
        values.put(COLUMN_ENABLE, if (adBlock.isEnable) 1 else 0)
        return try {
            db.update(table, values, "$COLUMN_ID = ?", arrayOf(adBlock.id.toString()))
            true
        } catch (e: SQLiteConstraintException) {
            false
        }
    ...
    companion object {
        ...
        private const val TYPE_DENY_TABLE = 1
        private const val TYPE_ALLOW_TABLE = 2
        private const val TYPE_ALLOW_PAGE_TABLE = 3
