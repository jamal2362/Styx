...
import com.jamal2367.styx.database.adblock.UserRulesRepository.Companion.RESPONSE_EXCLUSION
import com.jamal2367.styx.database.adblock.UserRulesRepository.Companion.RESPONSE_NOOP
    ...
    override fun removeRule(rule: UnifiedFilterResponse) {
        database.run {
            delete(TABLE_RULES,
                ...
                arrayOf(rule.response.toInt().toString(),
                    rule.filter.pattern,
                    rule.filter.domains?.toDBString() ?: "",
                    rule.filter.filterType.toString(),
                    rule.filter.contentType.toString(),
                    rule.filter.thirdParty.toString()))
        }
    }
