...
import com.jamal2367.styx.di.MainHandler
...
@AndroidEntryPoint
abstract class BrowserActivity : ThemedBrowserActivity(), BrowserView, UIController,
    ...
    @Inject
    lateinit var bookmarkPageInitializer: BookmarkPageInitializer
    @Inject
    @MainHandler
    ...
    @Inject
    lateinit var bookmarksDialogBuilder: StyxDialogBuilder
    @Inject
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (userPreferences.keepScreenOn) {
    ...
    private fun createMenuSessions() {
        sessionsMenu = SessionsPopupWindow(this, layoutInflater)
        // Make it full screen gesture friendly
        sessionsMenu.setOnDismissListener { justClosedMenuCountdown() }
    }
    ...
    private fun createTabsDialog() {
        ...
        tabsDialog.window?.setFlags(window.attributes.flags,
    ...
    private fun createBookmarksDialog() {
        ...
        bookmarksDialog.window?.setFlags(window.attributes.flags,
            WindowManager.LayoutParams.FLAG_FULLSCREEN)
        // Needed to make sure our bottom sheet shows below our session pop-up
    ...
    private fun initialize(savedInstanceState: Bundle?) {
        ...
        iBindingToolbarContent =
            ToolbarContentBinding.inflate(layoutInflater, iBinding.toolbarInclude.toolbar, true)
        ...
        iBinding.contentFrame.setOnRefreshListener {
            ...
            mainHandler.postDelayed({ iBinding.contentFrame.isRefreshing = false },
                1000)   // Stop the loading spinner after one second
        }
        ...
        iBinding.toolbarInclude.apply {
            ...
            if (userPreferences.toolbarsBottom) {
                ...
                bookmarksView?.apply {
                    ...
                    // Use reversed layout from bottom to top
                    (iBinding.listBookmarks.layoutManager as? LinearLayoutManager)?.reverseLayout =
                        true
                }
    ...
    override fun onKeyDown(keyCode: Int, event: KeyEvent): Boolean {
        if (keyCode == KeyEvent.KEYCODE_ENTER) {
            ...
            mainHandler.postDelayed(longPressBackRunnable,
    ...
    override fun dispatchKeyEvent(event: KeyEvent): Boolean {
        ...
        if (event.action == KeyEvent.ACTION_DOWN) {
            ...
            val isCtrlShiftOnly = KeyEvent.metaStateHasModifiers(event.metaState,
            ...
            if (isCtrlShiftOnly) {
                ...
                tabsManager.let {
                    if (KeyEvent.KEYCODE_0 <= event.keyCode && event.keyCode <= KeyEvent.KEYCODE_9) {
                        val nextIndex =
                            if (event.keyCode > it.iSessions.count() + KeyEvent.KEYCODE_1 || event.keyCode == KeyEvent.KEYCODE_0) {
            ...
            if (event.isCtrlPressed && event.keyCode == KeyEvent.KEYCODE_TAB) {
                ...
                iCapturedRecentTabsIndices?.let {
                    ...
                    if (event.isShiftPressed or event.isAltPressed or event.isFunctionPressed) {
                        ...
                        // Go back one tab
                        iRecentTabIndex--
                        if (iRecentTabIndex < 0) iRecentTabIndex =
                            iCapturedRecentTabsIndices?.size?.minus(1) ?: -1
                    }
    ...
    override fun executeAction(@IdRes id: Int): Boolean {
        ...
        when (id) {
            ...
            R.id.menuItemCopyLink -> {
                ...
                makeSnackbar(getString(R.string.message_link_copied),
            ...
            R.id.menuItemAdBlock -> {
                abpUserRules.allowPage(Uri.parse(tabsManager.currentTab?.url),
                    !popupMenu.iBinding.menuItemAdBlock.isChecked)
                tabsManager.currentTab?.reload()
                return true
            }
     ...
     *
     */
    private fun showFindInPageControls() {
    ...
    private fun setupPullToRefresh(configuration: Configuration) {
        ...
        iBindingToolbarContent.buttonReload.visibility =
            if (iBinding.contentFrame.isEnabled && !isLoading()) GONE else VISIBLE
    }
    ...
    override fun notifyTabViewRemoved(position: Int) {
        ...
        if (userPreferences.showUndo) {
            makeSnackbar(
                getString(R.string.notify_tab_closed),
                Snackbar.LENGTH_SHORT,
                if (userPreferences.toolbarsBottom || userPreferences.navbar) Gravity.TOP else Gravity.BOTTOM)
    ...
    override fun updateSslState(sslState: SslState) {
        iBindingToolbarContent.addressBarInclude.searchSslStatus.setImageDrawable(
            createSslDrawableForState(sslState))
    ...
    override fun setTabView(view: View) {
        ...
        currentTabView.onFocusLost {
            inputMethodManager.hideSoftInputFromWindow(iBinding.uiLayout.windowToken,
                0)
        }
        showActionBar()
        ...
        tabsManager.currentTab?.let {
            ...
            iSkipNextSearchQueryUpdate =
                true // Make sure we don't redo a search as our UI text is changed
    ...
    private fun applyToolbarColor(color: Int) {
        ...
        iBindingToolbarContent.addressBarInclude.searchSslStatus.setColorFilter(
            currentToolBarTextColor)
    ...
    private fun setSearchBarColors(aColor: Int) {
        iBindingToolbarContent.addressBarInclude.root.apply {
            ...
            stateListDrawable.drawableForState(android.R.attr.state_focused)
                .tint(ThemeUtils.getSearchBarFocusedColor(aColor))
            stateListDrawable.drawableForState(android.R.attr.state_enabled)
                .tint(ThemeUtils.getSearchBarColor(aColor))
        }
    }
        ...
        if (userPreferences.showUpdate) {
            ...
            val request = StringRequest(Request.Method.GET, url, { reply ->
                ...
                if (latestVersion != current) {
                    ...
                    makeCSnackbar(
                        getString(R.string.app_name) + " " + latestVersion + " " + getString(R.string.update_available),
                        10000,
                        if (userPreferences.toolbarsBottom || userPreferences.navbar) Gravity.TOP else Gravity.BOTTOM)
    ...
    /**
     * Welcome user with dialog after first installation.
     */
    private fun welcomeToStyx() {
        MaterialAlertDialogBuilder(this)
            ...
            .setNeutralButton(R.string.action_fulguris) { _, _ ->
                val fulgurisurl = getString(R.string.url_app_fulguris)
                val i = Intent(Intent.ACTION_VIEW)
                i.data = Uri.parse(fulgurisurl)
                i.putExtra("SOURCE", "SELF")
                startActivity(i)
            }
            .setPositiveButton(R.string.action_forward) { _, _ ->
                this.toast("❤️")
            }
            .resizeAndShow()
    }
    ...
    fun showPageToolsDialog(position: Int) {
        ...
        val jsEnabledString =
            if (userPreferences.javaScriptChoice == JavaScriptChoice.BLACKLIST && !stringContainsItemFromList(
                    currentTab.url,
                    strgs) || userPreferences.javaScriptChoice == JavaScriptChoice.WHITELIST && stringContainsItemFromList(
                    currentTab.url,
                    strgs)
            ) {
        ...
        BrowserDialog.showWithIcons(this, this.getString(R.string.dialog_tools_title),
            ...
            DialogItem(
                ...
                colorTint = this.attrColor(R.attr.colorPrimary).takeIf {
                    userPreferences.javaScriptChoice == JavaScriptChoice.BLACKLIST && !stringContainsItemFromList(
                        currentTab.url,
                        strgs) || userPreferences.javaScriptChoice == JavaScriptChoice.WHITELIST && stringContainsItemFromList(
                        currentTab.url,
                        strgs)
                ...
                if (userPreferences.javaScriptChoice != JavaScriptChoice.NONE) {
                    if (!stringContainsItemFromList(currentTab.url, strgs)) {
                        if (userPreferences.javaScriptBlocked == "") {
                            ...
                            userPreferences.javaScriptBlocked =
                                userPreferences.javaScriptBlocked + ", " + url.host
                        }
                    } else {
                        if (!userPreferences.javaScriptBlocked.contains(", " + url.host)) {
                            userPreferences.javaScriptBlocked =
                                userPreferences.javaScriptBlocked.replace(url.host, "")
                        } else {
                            userPreferences.javaScriptBlocked =
                                userPreferences.javaScriptBlocked.replace(", " + url.host, "")
                        }
