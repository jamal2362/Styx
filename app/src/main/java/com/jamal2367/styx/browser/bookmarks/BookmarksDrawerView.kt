...
class BookmarksDrawerView @JvmOverloads constructor(
    ...
    @Inject
    internal lateinit var bookmarksDialogBuilder: StyxDialogBuilder
    @Inject
    ...
    var iBinding: BookmarkDrawerViewBinding =
        BookmarkDrawerViewBinding.inflate(context.inflater, this, true)
    ...
    init {
        ...
        val callback: ItemTouchHelper.Callback =
            ItemDragDropSwipeHelper(iAdapter, aLongPressDragEnabled = true, aSwipeEnabled = false)
    ...
    private fun updateBookmarkIndicator(url: String) {
        ...
        bookmarkUpdateSubscription = bookmarkModel.isBookmark(url)
            ...
            .subscribe { isBookmark ->
                ...
                addBookmarkView?.isEnabled =
                    !url.isSpecialUrl() && !url.isHomeUri() && !url.isBookmarkUri() && !url.isHistoryUri()
            }
    }
    ...
    private fun setBookmarksShown(folder: String?, animate: Boolean) {
        ...
        bookmarksSubscription = bookmarkModel.getBookmarksFromFolderSorted(folder)
            ...
            .subscribe { bookmarksAndFolders ->
                ...
                iBinding.textTitle.text =
                    if (folder.isNullOrBlank()) resources.getString(R.string.action_bookmarks) else folder
            }
    }
    ...
    private fun showBookmarkMenu(bookmark: Bookmark): Boolean {
        (context as AppCompatActivity?)?.let {
            when (bookmark) {
                is Bookmark.Folder -> bookmarksDialogBuilder.showBookmarkFolderLongPressedDialog(it,
                    uiController,
                    bookmark)
                is Bookmark.Entry -> bookmarksDialogBuilder.showLongPressedDialogForBookmarkUrl(it,
                    uiController,
                    bookmark)
            }
        }
    ...
    private fun openBookmark(bookmark: Bookmark) = when (bookmark) {
        is Bookmark.Folder -> {
            scrollIndex =
                (iBinding.listBookmarks.layoutManager as LinearLayoutManager).findFirstVisibleItemPosition()
