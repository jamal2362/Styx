...
class SearchEngineProvider @Inject constructor(
    ...
    fun provideSearchSuggestions(): SuggestionsRepository =
        when (userPreferences.searchSuggestionChoice) {
            ...
            1 -> GoogleSuggestionsModel(okHttpClient,
                requestFactory,
                application,
                logger,
                userPreferences)
            2 -> DuckSuggestionsModel(okHttpClient,
                requestFactory,
                application,
                logger,
                userPreferences)
            3 -> BaiduSuggestionsModel(okHttpClient,
                requestFactory,
                application,
                logger,
                userPreferences)
            4 -> NaverSuggestionsModel(okHttpClient,
                requestFactory,
                application,
                logger,
                userPreferences)
            else -> GoogleSuggestionsModel(okHttpClient,
                requestFactory,
                application,
                logger,
                userPreferences)
