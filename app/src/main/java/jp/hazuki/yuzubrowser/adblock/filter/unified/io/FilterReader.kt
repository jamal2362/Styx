...
class FilterReader(private val input: InputStream) {
    ...
    private fun readModify(): ModifyFilter? {
        ...
        return when (modifyString[0]) {
            ...
            MODIFY_PREFIX_REMOVEPARAM_REGEX -> RemoveparamRegexFilter(modifyParameter
                ?: return null, modifyString[1] == '1')
            MODIFY_PREFIX_REDIRECT -> RedirectFilter(modifyParameter)
            MODIFY_PREFIX_REQUEST_HEADER -> RequestHeaderFilter(modifyParameter ?: return null,
                modifyString[1] == '1')
            MODIFY_PREFIX_RESPONSE_HEADER -> ResponseHeaderFilter(modifyParameter ?: return null,
                modifyString[1] == '1')
            else -> null
        }
    }
    ...
    private fun readFilter(modify: ModifyFilter? = null): Pair<String, UnifiedFilter>? {
        ...
        if (type == FILTER_TYPE_START_END_DOMAIN)
            return (Pair(pattern,
                StartEndFilter(pattern, contentType, false, null, thirdParty, modify)))
        ...
        val filter = when (type) {
            FILTER_TYPE_CONTAINS -> ContainsFilter(pattern,
                contentType,
                ignoreCase,
                domains,
                thirdParty,
            ...
            FILTER_TYPE_CONTAINS_HOST -> ContainsHostFilter(pattern,
                contentType,
                domains,
                thirdParty,
                modify)
            FILTER_TYPE_START -> StartsWithFilter(pattern,
                contentType,
                ignoreCase,
                domains,
                thirdParty,
                modify)
            FILTER_TYPE_END -> EndWithFilter(pattern,
                contentType,
                ignoreCase,
                domains,
                thirdParty,
                modify)
            FILTER_TYPE_START_END -> StartEndFilter(pattern,
                contentType,
                ignoreCase,
                domains,
                thirdParty,
                modify)
            FILTER_TYPE_JVM_REGEX -> RegexFilter(pattern,
                contentType,
                ignoreCase,
                domains,
                thirdParty,
                modify)
            FILTER_TYPE_JVM_REGEX_HOST -> RegexHostFilter(pattern,
                contentType,
                ignoreCase,
                domains,
                thirdParty,
                modify)
            FILTER_TYPE_PATTERN -> PatternMatchFilter(pattern,
                contentType,
                ignoreCase,
                domains,
                thirdParty,
