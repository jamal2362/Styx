...
import com.jamal2367.styx.adblock.RES_EMPTY
import com.jamal2367.styx.adblock.RES_NOOP_MP4
...
class AbpFilterDecoder {
    ...
    fun decode(reader: BufferedReader): UnifiedFilterSet {
        ...
        reader.forEachLine { line ->
            ...
            when {
                trimmedLine[0] == '!' -> trimmedLine.decodeComment(info)
                else -> {
    ...
    private fun String.decodeFilter(
        ...
        if (responseHeaderStart > -1) {
            ...
            filter = if (other.length > 2 && other[2] == '$')
                filter.substring(0,
                    responseHeaderStart) + "\$removeheader=" + header + "," + other.substring(2)
            else
...
*/
                ...
                when {
                    ...
                    type == 0 -> {
                        when (option) {
                            ...
                            "third-party", "3p" -> thirdParty =
                                if (inverse) FIRST_PARTY else THIRD_PARTY
                            "first-party", "1p" -> thirdParty =
                                if (inverse) THIRD_PARTY else FIRST_PARTY
                            "strict3p" -> thirdParty =
                                if (inverse) STRICT_FIRST_PARTY else STRICT_THIRD_PARTY
                            "strict1p" -> thirdParty =
                                if (inverse) STRICT_THIRD_PARTY else STRICT_FIRST_PARTY
                            "sitekey" -> Unit
                            "removeparam", "queryprune" -> {
                                modify = if (value == null || value.isEmpty()) RemoveparamFilter(
                                    null,
                                    false)
                                else {
                                    if (value.startsWith('~'))
                            ...
                            "csp" -> {
                                modify =
                                    if (value == null) ResponseHeaderFilter("Content-Security-Policy",
                                        false)
                                    else ResponseHeaderFilter("Content-Security-Policy: ${
                                        value.substringAfter('=')
                                    }", false)
                                contentType =
                                    contentType or (ContentRequest.TYPE_DOCUMENT and ContentRequest.TYPE_SUB_DOCUMENT) // uBo documentation: It can be applied to main document and documents in frames
                            }
                            "inline-font" -> {
                                // header value from uBlock source, src/js/background.js
                                modify = ResponseHeaderFilter("Content-Security-Policy: font-src *",
                                    false)
                                contentType =
                                    contentType or (ContentRequest.TYPE_DOCUMENT and ContentRequest.TYPE_SUB_DOCUMENT)
                            }
                            "inline-script" -> {
                                // header value from uBlock source, src/js/background.js
                                modify =
                                    ResponseHeaderFilter("Content-Security-Policy: script-src 'unsafe-eval' * blob: data:",
                                        false)
                                contentType =
                                    contentType or (ContentRequest.TYPE_DOCUMENT and ContentRequest.TYPE_SUB_DOCUMENT)
                            }
                            "removeheader" -> {
                                ...
                                val header =
                                    if (request) value.substringAfter("request:") else value
                            ...
                            "redirect" -> {
                                ...
                                this.removeOption("redirect")
                                    .decodeFilter(elementFilterList, filterLists)
                            ...
                            "mp4" -> {
                                this.removeOption("mp4")
                                    .decodeFilter(elementFilterList, filterLists)
                                modify = RedirectFilter(RES_NOOP_MP4)
                                contentType =
                                    contentType or ContentRequest.TYPE_MEDIA // uBo documentation: media type will be assumed
                            }
                            "important" -> important = true
                            // TODO: see above, all is not handled 100% correctly (but might still be fine)
                            "all" -> contentType =
                                contentType or ContentRequest.TYPE_DOCUMENT or ContentRequest.TYPE_STYLE_SHEET or ContentRequest.TYPE_IMAGE or ContentRequest.TYPE_OTHER or ContentRequest.TYPE_SCRIPT or ContentRequest.TYPE_XHR or ContentRequest.TYPE_FONT or ContentRequest.TYPE_MEDIA or ContentRequest.TYPE_WEB_SOCKET
                            ...
                            "denyallow" -> {
                                ...
                                value.split('|').forEach { denyAllowDomain ->
                                    "@@||$denyAllowDomain\$$optionsString".decodeFilter(
                                        elementFilterList,
                                        filterLists)
                                }
                            }
        ...
        val abpFilter =
            if (filter.length >= 2 && filter[0] == '/' && filter[filter.lastIndex] == '/' && filter.mayContainRegexChars()) {
                RegexFilter(filter.substring(1, filter.lastIndex),
                    contentType,
                    ignoreCase,
                    domains,
                    thirdParty,
                ...
                if (isLiteral) {
                    when {
                        isStartsWith && isEndWith -> StartEndFilter(content,
                            contentType,
                            ignoreCase,
                            domains,
                            thirdParty,
                            modify)
                        isStartsWith -> StartsWithFilter(content,
                            contentType,
                            ignoreCase,
                            domains,
                            thirdParty,
                            modify)
                        isEndWith -> EndWithFilter(content,
                            contentType,
                            ignoreCase,
                            domains,
                            thirdParty,
    ...
    private fun String.decodeComment(info: DecoderInfo) {
        // comment format:
        // ! <title>: <content>
        if (!contains(':')) return
