...
import com.jamal2367.styx.adblock.AbpBlockerManager.Companion.badfilterPrefixes
...
class FilterMap {
    private val map =
        (blockerPrefixes + badfilterPrefixes).associateWith { mutableListOf<UnifiedFilter>() }

    operator fun get(list: String) =
        map[list] ?: throw(InvalidParameterException("list $list does not exist"))
}
